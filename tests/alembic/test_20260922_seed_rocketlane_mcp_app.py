"""Tests for the provenance-safe Rocketlane remote-MCP connector seed."""
...
def _load_migration():
    path = (
        ...
        / "src/xagent/migrations/versions/20260922_seed_rocketlane_mcp_app.py"
    )
    spec = importlib.util.spec_from_file_location("seed_rocketlane_migration", path)
...
def test_seed_row_endpoint_yields_the_verified_discovery_url():
    """The endpoint path is load-bearing, which the miro/atlassian rows'
    tests have no equivalent of: Rocketlane publishes its protected-resource
    metadata only under /mcp (the host root returns 404), so the first
    candidate our discovery builds from this row's URL must be exactly the
    document verified against the vendor. A trimmed or rewritten path would
    still look like a valid endpoint and would fail the connect flow at
    discovery instead."""
    from xagent.web.services.mcp_oauth import protected_resource_metadata_urls

    migration = _load_migration()
    candidates = protected_resource_metadata_urls(
        str(migration.ROW["launch_config"]["url"])
    )
    assert candidates[0] == (
        "https://rocket-mcp.rl-platforms.rocketlane.com"
