"""seed built-in Rocketlane (remote MCP, DCR OAuth) connector

Revision ID: 20260920_seed_rocketlane_mcp_app
Revises: 20260920_seed_fireflies_mcp_app
...
revision: str = "20260920_seed_rocketlane_mcp_app"
down_revision: Union[str, None] = "20260920_seed_fireflies_mcp_app"
...
# Rocketlane serves MCP at https://rocket-mcp.rl-platforms.rocketlane.com/mcp;
# its protected-resource metadata delegates to a Scalekit-hosted authorization
# server (rocketlane.scalekit.com) that advertises a registration_endpoint and
...
# No mcp_servers-table provenance check (unlike the shopify seed): that check
# guards shopify's key-based connect_mcp_app path, whose MCPServer.auth carries
# the same marker. Remote mcp_oauth apps activate through
# _ensure_catalog_mcp_oauth_server (api/mcp.py), which only copies
# launch_config.url/auth onto the shared server row and already rejects a
# same-named server whose transport or URL differs from the catalog -- an
# independent, pre-existing guard every mcp_oauth builtin app relies on.
...
def _collides_with_rocketlane_identity(value: object) -> bool:
...
def upgrade() -> None:
    ...
    if "launch_config" not in columns:
        raise RuntimeError(
            "Cannot seed builtin Rocketlane identity: public_mcp_apps.launch_config "
    ...
    if exact_app_rows:
        ...
        raise RuntimeError(
            "Cannot seed builtin Rocketlane connector: an existing "
            "public_mcp_apps row with app_id='rocketlane' has no matching "
            "builtin_provenance"
        )

    # No row claims app_id "rocketlane". A *different* app_id whose display name
    ...
    colliding_rows = [
        ...
        if _collides_with_rocketlane_identity(row["app_id"])
        or _collides_with_rocketlane_identity(row["name"])
    ]
    if colliding_rows:
        logger.error(
            "Permanently skipping builtin Rocketlane seed: public_mcp_apps "
...
def downgrade() -> None:
    ...
    # exists for Rocketlane (auth is per-user Dynamic Client Registration, not a
