...
def _reconcile_mcp_servers(bind: sa.engine.Connection, inspector: sa.Inspector) -> bool:
    ...
    if (
        ...
        logger.error(
            "Permanently skipping builtin Miro seed: mcp_servers row(s) %s "
    ...
    if len(colliding) == 1:
        ...
        if (
            ...
            return True
    logger.error(
        "Permanently skipping builtin Miro seed: mcp_servers row(s) %s collide "
...
def downgrade() -> None:
    ...
    # oauth_providers row exists for Miro (auth is per-user Dynamic Client
