"""seed built-in Fireflies (remote MCP, DCR OAuth) connector

Revision ID: 20260920_seed_fireflies_mcp_app
Revises: 20260920_seed_miro_mcp_app
...
revision: str = "20260920_seed_fireflies_mcp_app"
down_revision: Union[str, None] = "20260920_seed_miro_mcp_app"
...
# Fireflies serves MCP at https://api.fireflies.ai/mcp; its protected-resource
# metadata names "https://api.fireflies.ai/" as the authorization server, which
# advertises a registration_endpoint and token_endpoint_auth_method "none".
ROW = {
    "app_id": APP_ID,
    "name": "Fireflies",
...
def _collides_with_fireflies_identity(value: object) -> bool:
...
def upgrade() -> None:
    ...
    if "launch_config" not in columns:
        raise RuntimeError(
            "Cannot seed builtin Fireflies identity: public_mcp_apps.launch_config "
    ...
    if exact_app_rows:
        ...
        raise RuntimeError(
            "Cannot seed builtin Fireflies connector: an existing "
            "public_mcp_apps row with app_id='fireflies' has no matching "
            "builtin_provenance"
        )

    # No row claims app_id "fireflies". A *different* app_id whose display name
    ...
    colliding_rows = [
        ...
        if _collides_with_fireflies_identity(row["app_id"])
        or _collides_with_fireflies_identity(row["name"])
    ]
    if colliding_rows:
        logger.error(
            "Permanently skipping builtin Fireflies seed: public_mcp_apps "
...
def downgrade() -> None:
    ...
    # exists for Fireflies (auth is per-user Dynamic Client Registration, not a
